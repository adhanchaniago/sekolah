...
class Simple_login {
	...
	public function login($username, $password) {
		$query = $this->CI->db->get_where('web_user',array('username'=>$username,'password' => $password));
		if($query->num_rows() == 1) {
			$admin 	= $query->row();
			$newdata = array(
		        'id'  		=> $admin->id,
			...
			$this->CI->session->set_flashdata('message','<p class="text-danger"> Oops... Periksa Username dan Password anda</p>');
